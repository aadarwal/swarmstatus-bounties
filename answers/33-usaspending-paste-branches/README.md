...
1. **Source-qualified external template / task publication:** Unrecovered / Inconclusive. Current evidence is restricted to the seven retained first-party captures from `paste.linuxiarz.pl`. No external task publication, source template, or dated artifact outside the known set was recovered. Displayed relative ages (“3 Months ago”) are preserved without conversion to UTC timestamps.
2. **Exact ordered correspondence & distinctive ID connection:** Supported for exact ordered correspondence within the retained pairs; Inconclusive for external task connection. The four-link API block (`ACCGET`, `ACCGET23`, `SNAP23`, `QEND`) and seven-link SF133 block (`Q2MDSCHEMELESS`, `Q3MDSCHEMELESS`, `Q2DIRECT`, `Q3DIRECT`, `Q2MDFULL`, `Q2CORS`, `Q2AO`) match across sibling branches under filler parents `x` and `SHIFT8`. Query order, spelling, label order, and nested wrapper representations are preserved verbatim. Distinctive account IDs (`075-8005`, `5599`) and MAX attachment IDs (`2346466575`, `2374423602`, `2398882076`) are verified, but an external template or task connecting them remains unrecovered.
3. **Origin and propagation direction:** Unclaimed / Bounded. No origin or propagation direction is asserted. Text equality, relative ages, and provider reply tables demonstrate structural reuse only, not temporal priority, semantic exchange, actor identity, or causal direction.
4. **Reproducible offline comparisons & negative controls:** Supported. Standalone script `verify.py --negative-controls` passes deterministically offline without network or database dependencies. Ten negative controls reject tampered bytes, swapped parent assertions, altered reciprocal reply rows, altered account IDs (`5599` -> `5598`), altered attachment IDs (`2398882076` -> `2398882077`), swapped URL/label order, ordinary documentation examples (`2357`/`2015`), promotion of filler to task packets, false RefNX pairing, and rewritten URL-encoded delimiters (`%26` -> `&`).
5. **Artifact text, requested URLs, and returned data separation:** Supported. Retained paste bodies are decoded once from literal `textarea#code`. Requested endpoints and proxy wrappers (`markdown.new`, `test.cors.workers.dev`, `allorigins.hexlet.app`) are preserved as literal strings without fetching or replaying remote responses. No government-data response is asserted or implied.
