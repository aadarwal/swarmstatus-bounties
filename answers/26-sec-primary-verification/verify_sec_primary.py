...
def select_transaction(raw):
    """Select only relevant fields; never expose unrelated cookies/identifiers."""
...
def verify(primary_bytes, cached, receipt, capture, raw_receipt_path=None):
    ...
    for year, (start, end) in WINDOWS.items():
        ...
        for row in expected:
            ...
            # Equivalent to the retained positive-value jq idiom, without running jq.
